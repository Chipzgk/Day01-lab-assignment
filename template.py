...
# Nạp biến môi trường cho toàn bộ file ngay từ đầu
load_dotenv()
...
def compare_models(prompt: str) -> dict:
    ...
    openai_response, openai_latency = call_openai(prompt)
    mini_response, mini_latency = call_openai_mini(prompt)

    cost = (len(openai_response.split()) / 0.75) / 1000 * COST_PER_1K_OUTPUT_TOKENS["gpt-4o"]
    # (0.75 words ≈ 1 token is a rough approximation)

    return {
        "gpt4o_response": openai_response,
        "mini_response": mini_response,
        "gpt4o_latency": openai_latency,
...
def streaming_chatbot() -> None:
    ...
    while True:
        try:
            # Khối try-except giúp pytest test việc gõ 'quit' không bị sập
            user_input = input("\nYou: ")
        except EOFError:
            break
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for res in results:
        prompt_trunc = (res["prompt"][:37] + "...") if len(res["prompt"]) > 40 else res["prompt"]
        gpt4o_trunc = (res["gpt4o_response"][:37] + "...") if len(res["gpt4o_response"]) > 40 else res["gpt4o_response"]
        mini_trunc = (res["mini_response"][:37] + "...") if len(res["mini_response"]) > 40 else res["mini_response"]
        
        row = f"{prompt_trunc:<42} | {gpt4o_trunc:<42} | {mini_trunc:<42} | {res['gpt4o_latency']:<15.2f} | {res['mini_latency']:<15.2f}"
        rows.append(row)
